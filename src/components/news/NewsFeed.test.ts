...
describe('NewsFeed states', () => {
  ...
  it('renders only HTTPS hosts explicitly allowed by CSP', () => {
    expect(shouldRenderNewsImage(true, 'https://picsum.photos/image.jpg')).toBe(false);
    expect(shouldRenderNewsImage(false, 'https://example.com/image.jpg')).toBe(false);
    expect(shouldRenderNewsImage(false, 'https://picsum.photos/image.jpg')).toBe(true);
  });
