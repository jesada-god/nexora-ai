...
export function NewsFeed({ symbol, compact = false }: { symbol?: string; compact?: boolean }) {
  ...
  else if (error && items.length === 0) content = <div className="rounded-xl border border-slate-800 bg-slate-900/30 p-6 text-center"><Newspaper className="mx-auto mb-2 text-slate-500" /><p className="text-sm text-slate-300">{newsErrorMessage(error.code)}</p>{error.retryable && <button disabled={cooldown > 0} onClick={() => void fetchPage(undefined, true)} className="mt-3 rounded-lg border border-slate-700 px-3 py-2 text-xs text-white disabled:opacity-50">{cooldown ? `ลองใหม่ใน ${cooldown} วินาที` : 'ลองใหม่'}</button>}</div>;
  ...
  else content = <div className="space-y-3">{timestamp && <p className="text-right text-[10px] text-slate-500">อัปเดต {new Date(timestamp).toLocaleString('th-TH')}</p>}{items.slice(0, compact ? 4 : undefined).map((article) => <a key={article.id} href={article.url} target="_blank" rel="noopener noreferrer" className="flex min-h-24 gap-3 rounded-xl border border-slate-800 bg-[#151B28] p-3 hover:border-slate-700">{shouldRenderNewsImage(saveData, article.imageUrl) ? <img src={article.imageUrl!} alt="" loading="lazy" referrerPolicy="no-referrer" className="h-20 w-24 shrink-0 rounded-lg object-cover" /> : <div className="flex h-20 w-24 shrink-0 items-center justify-center rounded-lg bg-slate-800 text-slate-500"><ImageOff size={20} /></div>}<div className="min-w-0"><h3 className="line-clamp-2 text-sm font-semibold leading-snug text-slate-100">{article.title}</h3><p className="mt-2 text-xs text-slate-500">{article.source} · {new Intl.DateTimeFormat('th-TH', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(article.publishedAt))}</p>{saveData && <span className="mt-1 inline-block text-[10px] text-amber-300">Data Saver</span>}</div></a>)}{!compact && cursor && <button disabled={loading} onClick={() => void fetchPage(cursor)} className="w-full rounded-xl border border-slate-700 bg-slate-800/50 px-4 py-2 text-sm text-white disabled:opacity-50">{loading ? 'กำลังโหลด…' : 'โหลดเพิ่มเติม'}</button>}</div>;
