...
function levelTooltip(level: ChartLevel) {
  const components = Object.entries(level.scoreComponents)
    .map(([name, value]) => `${name}=${value == null ? 'unavailable' : value.toFixed(2)}`)
    .join(', ');
  return `${level.label}: score ${level.score.toFixed(1)}, touches ${level.touches}, last touch ${level.latestTouchAt}; ${components}`;
}
...
export default function HistoricalChart({ prices, symbol, visibleBarCount = DEFAULT_VISIBLE_BARS, technical, enabledIndicators = [], chartType = 'area', supportResistance, volumeProfile, fibonacci, showVolume = true, showVpvr = false, showFibonacci = false }: Props) {
  ...
  const priceChart = <div className={fullscreen ? 'h-[58dvh] min-h-[22rem]' : 'h-[20rem] md:h-[25rem]'}><ResponsiveContainer width="100%" height="100%"><ComposedChart data={visibleData} syncId="nexora-professional-chart" syncMethod="index" margin={CHART_MARGIN} barCategoryGap="18%" onMouseMove={onMouseState} onMouseLeave={() => { if (!inspectionLocked && !dragRef.current) setCrosshairTime(null); }}>
    ...
    <CartesianGrid stroke="#1e293b"/><XAxis dataKey="date" hide/><YAxis yAxisId="price" orientation="right" width={PRICE_AXIS_WIDTH} domain={['auto', 'auto']} tick={{ fontSize: 10 }} stroke="#64748b"/><Tooltip content={() => null} cursor={false}/><Legend verticalAlign="top" height={28}/>
    {sharedCrosshair}{crosshairPoint && <ReferenceLine yAxisId="price" y={crosshairPoint.raw.close} stroke="#64748b" strokeDasharray="2 4"/>}
    <ReferenceLine yAxisId="price" y={latest.close} stroke="#D4FF00" strokeDasharray="6 4" label={{ value: price(latest.close), position: 'insideRight', fill: '#D4FF00', fontSize: 10 }}/>
    {srLevels.map((level) => <ReferenceLine yAxisId="price" key={level.id} y={level.price} stroke={levelColor(level)} strokeDasharray="5 4" label={{ value: `${level.label} ${price(level.price)}`, position: levelLabelPosition(level), fill: levelColor(level), fontSize: 9 }}/>) }
