...
export function ChartPanel({ symbol, active }: { symbol: string; active: boolean }) {
  const [range, setRange] = useState<HistoricalRange>('3m'); const [result, setResult] = useState<{ key: string; response: HistoryResponse } | null>(null);
  ...
  const startRequest = useCallback((targetSymbol: string, targetRange: HistoricalRange, reason: 'selection' | 'retry') => {
    if (!canLoadHistory(active, document.visibilityState)) return;
  ...
  useEffect(() => {
    if (!active || !canLoadHistory(true, document.visibilityState)) return;
  ...
  }, [active, range, session, startRequest, symbol]);
  ...
  return <div><div className="mb-3 flex items-center gap-1 overflow-x-auto">{(['1m','3m','6m','1y','5y','max'] as HistoricalRange[]).map((value) => <button key={value} onClick={() => setRange(value)} className={`min-w-12 rounded-full px-3 text-xs ${range === value ? 'bg-[#D4FF00] text-black' : 'bg-slate-800 text-slate-300'}`}>{value.toUpperCase()}</button>)}<div className="ml-auto shrink-0 text-right"><span className="rounded-full border border-slate-700 px-2 py-1 text-[10px] uppercase text-slate-400">{loading && !current ? 'loading' : current?.meta.freshness.status ?? 'ready'}</span>{timestamp && <p className="mt-1 text-[10px] text-slate-500">{new Date(timestamp).toLocaleString('th-TH')}</p>}</div></div><div className="relative h-[300px] md:h-[420px]">{current?.data ? <Chart prices={current.data.prices}/> : current?.error ? <div className="flex h-full flex-col items-center justify-center gap-3 rounded-xl border border-red-500/20 px-4 text-center text-sm text-red-300"><p>{historyErrorMessage(current.error.code)}</p>{current.error.retryable && <button disabled={loading || cooldown > 0} onClick={() => startRequest(symbol, range, 'retry')} className="rounded-lg border border-slate-700 px-3 py-2 text-slate-200 disabled:opacity-50">{loading ? 'กำลังโหลด…' : cooldown ? `ลองใหม่ใน ${cooldown} วินาที` : 'ลองใหม่'}</button>}</div> : <Skeleton className="h-full w-full rounded-xl" />}</div></div>;
}
