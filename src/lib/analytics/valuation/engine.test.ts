...
describe('Nexora Composite Fair Value engine', () => {
  ...
  it('returns typed unavailable rather than defaults when inputs are insufficient', () => { const result = calculateFairValue({ ...input, periods: [] }, Date.parse('2026-01-03')); expect(result).toMatchObject({ status: 'unavailable', failureKind: 'insufficient-data', provider: 'verified-fixture', missingFields: expect.any(Array), asOf: input.priceAsOf, methodologyVersion: 'nexora-fv-v1' }); if (result.status === 'unavailable') expect(result.missingInputs.length).toBeGreaterThan(0); });
