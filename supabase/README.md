...
Phase 9 background alerts additionally use the Service Role only inside the authenticated cron route. Apply `202607180010_phase_9_background_alerts_push.sql`, configure `CRON_SECRET` and VAPID server variables, then invoke `GET /api/cron/alerts` with `Authorization: Bearer <CRON_SECRET>`. Browser and normal user actions continue to use the publishable key plus RLS; `push_deliveries` and `alert_evaluation_runs` have no authenticated-user policies.
